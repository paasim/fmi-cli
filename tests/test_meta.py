...
def test_stations():
    ...
    assert "100971" in weather_helsinki_ids  # kaisaniemi
    assert "100968" in weather_helsinki_ids  # helsinki-vantaa

    airquality_ids = [s.fmisid for s in stations.airquality()]
    assert "100662" in airquality_ids  # helsinki kallio 2
    assert "101942" in airquality_ids  # sodankylä heikinheimo

    radiation_ids = [s.fmisid for s in stations.radiation()]
    assert "101004" in radiation_ids  # helsinki kumpula
    assert "100968" in radiation_ids  # helsinki-vantaa
